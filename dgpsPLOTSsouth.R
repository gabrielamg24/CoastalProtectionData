...
pdf("dgpsPLOTSsouth.pdf", width=6 , height=4 , paper='special')
...
ggplot(d.summary, aes(x = meters, y = ymean, color = time, shape = time)) +
  ...
  scale_y_continuous(limits = c(-0.4, -0.10)) +
  labs(title= "Sediment accretion in SOUTHERN plots", subtitle= "standard deviation", x= "distance (meters)", y="sediment height", color="Treatment", shape="Treatment") +
...
ggplot(data = south, aes(x = meters, y = mean, color = time, shape=time)) +
  ...
  ggtitle("Sediment accretion SOUTH ") +
  theme_light (base_size = 12, base_family = "") + 
